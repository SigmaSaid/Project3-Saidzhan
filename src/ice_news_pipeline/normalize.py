...
def normalize_document(doc: ICEDocument) -> ICEDocument:
    ...
    body = normalize_text(doc.body_text, preserve_lines=True) or ""
    ...
    return ICEDocument(
        ...
        dateline_raw=normalize_text(doc.dateline_raw),
        dateline_city=normalize_text(doc.dateline_city),
        dateline_region=normalize_text(doc.dateline_region),
        city=normalize_text(doc.city),
        state=normalize_text(doc.state),
        # content
        full_text=normalize_text(doc.full_text, preserve_lines=True) or "",
        body_text=body,
        paragraphs=[normalize_text(p) or "" for p in doc.paragraphs],
        word_count=len(body.split()),
        # media
        image_urls=[u for u in (normalize_url(img) for img in doc.image_urls) if u],
        # parsing
        ...
        # audit
        field_provenance=doc.field_provenance,
